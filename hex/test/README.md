...
- `unclassified`: proposal tests. They intentionally make no authority claim
  yet; they must be promoted, rewritten, or deleted before they become design
  evidence.
...
Class-c and unclassified tests are visible, but they should not become evidence
in class-a architecture checks until they are promoted.
...
Unclassified files are the holding area for possible product laws before we
...
- `unclassified` to any class: promote only after deciding the test's authority
  level. If that decision is unclear, leave the test unclassified.
