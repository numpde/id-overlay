...
test("application source does not read ambient runtime facts", () => {
  ...
  trace.edge(flowEdge("check.application-ambient-read-boundary", "sink.architecture-boundary", {
...
test("application source does not execute effects", () => {
  ...
  trace.edge(flowEdge("check.application-effect-execution-boundary", "sink.architecture-boundary", {
    terminal: "architecture-check",
  }));
});
