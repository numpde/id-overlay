...
test("domain, application, and ports do not mention browser or platform objects", () => {
  ...
  trace.edge(flowEdge("check.core-browser-independence", "sink.architecture-boundary", {
...
test("domain and application tests stay plain-data tests", () => {
  ...
  assert.deepEqual(violations, []);
  trace.edge(flowEdge("check.pure-core-test-browser-independence", "sink.architecture-boundary", {
    terminal: "architecture-check",
  }));
});
