...
test("application boundary error exposes stable identity and code", () => {
  const trace = createBoundaryTrace("application boundary error exposes stable identity and code");
  ...
  trace.edge(boundaryCheckEdge("check.application-boundary-error-vocabulary", "stable-identity"));
});
...
test("unknown commands throw ApplicationBoundaryError", () => {
  const trace = createBoundaryTrace("unknown commands throw ApplicationBoundaryError");
  ...
  trace.edge(boundaryCheckEdge("check.application-command-boundary", "unknown-command"));
});
...
test("known commands with malformed payloads throw ApplicationBoundaryError", () => {
  const trace = createBoundaryTrace("known commands with malformed payloads throw ApplicationBoundaryError");
  ...
  trace.edge(boundaryCheckEdge("check.application-command-boundary", "malformed-payload"));
});
...
test("select-mode rejects caller-supplied solved placement", () => {
  const trace = createBoundaryTrace("select-mode rejects caller-supplied solved placement");
  ...
  trace.edge(boundaryCheckEdge("check.application-command-boundary", "select-mode-placement-smuggling"));
});
...
test("accepted reference-image input outcome requires normalized reference image data", () => {
  const trace = createBoundaryTrace("accepted reference-image input outcome requires normalized reference image data");
  ...
  trace.edge(boundaryCheckEdge("check.reference-image-input-boundary", "normalized-reference-image-data"));
});
...
test("malformed reference-image input outcome commands are boundary errors", () => {
  const trace = createBoundaryTrace("malformed reference-image input outcome commands are boundary errors");
  ...
  trace.edge(boundaryCheckEdge("check.reference-image-input-boundary", "malformed-input-outcome"));
});
...
test("source-specific input failure reasons are invalid application commands", () => {
  const trace = createBoundaryTrace("source-specific input failure reasons are invalid application commands");
  ...
  trace.edge(boundaryCheckEdge("check.reference-image-input-boundary", "source-specific-failure-reason"));
});
...
test("invalid application state throws ApplicationBoundaryError", () => {
  const trace = createBoundaryTrace("invalid application state throws ApplicationBoundaryError");
  ...
  trace.edge(boundaryCheckEdge("check.application-state-boundary", "invalid-application-state"));
});

function createBoundaryTrace(testName) {
...
function boundaryCheckEdge(from, phase) {
  return flowEdge(from, "sink.application-boundary-error", {
    phase,
