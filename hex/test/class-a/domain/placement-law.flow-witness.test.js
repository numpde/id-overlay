...
test("placement transform round-trips through its inverse", () => {
  ...
  trace.edge(flowEdge("check.placement-round-trip", "sink.domain-law", {
...
test("placement edit composition is stable across batching", () => {
  ...
  trace.edge(flowEdge("check.placement-edit-composition", "sink.domain-law", {
    terminal: "domain-result",
  }));
});
...
test("anchored placement edits keep the anchor fixed in screen space", () => {
  ...
  trace.edge(flowEdge("check.anchored-placement-edit", "sink.domain-law", {
    terminal: "domain-result",
  }));
});
