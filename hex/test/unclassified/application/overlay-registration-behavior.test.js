...
// Unclassified: placement edits are user-visible overlay edits and should be
// durable semantic history. The current minimal reducer keeps them inert; this
// test names the intended app-level behavior before implementation.
test("committed move rotate and scale placement edits update durable placement history", () => {
  for (const edit of [
    {
      kind: "move",
      placement: {
    ...
    const result = step(loadedState(), createApplicationCommand(
      APPLICATION_COMMAND_KIND.COMMIT_PLACEMENT_EDIT,
      edit,
    ));

    assert.deepEqual(result.state.session.placement, edit.placement);
    assert.deepEqual(result.state.history.past.at(-1), {
      kind: `${edit.kind}-overlay`,
